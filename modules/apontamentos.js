...
async function processarImport(file){
  ...
  try{
    ...
    function parseData(v){if(!v&&v!==0)return null;if(typeof v==='number'&&v>10000)return serialToIso(v);const s=String(v).trim();if(reData.test(s)){const[d,m,y]=s.split('/');return `${y.length===2?'20'+y:y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`;}return null;}
    function parseHora(v){if(!v&&v!==0)return '';if(typeof v==='number'){const tm=Math.round(v*1440);const h=Math.floor(tm/60),mn=tm%60;return `${String(h).padStart(2,'0')}:${String(mn).padStart(2,'0')}`;}return String(v).trim();}
    function parseHH(v){if(!v&&v!==0)return 0;if(typeof v==='number'&&v<1)return parseFloat((v*24).toFixed(2));return parseFloat(String(v).replace(',','.'))||0;}
    ...
    if(!records.length){showToast('Nenhum registro encontrado.','erro');prog.style.display='none';return;}
    setP(30,`${records.length} apontamentos. Enviando…`);
    const LOTE=200;
    ...
    setP(100,'Concluído!');
    showToast(`${records.length} apontamentos importados.`,'ok');
